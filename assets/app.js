(() => {
  ...
  function card(item, compact=false){
    ...
    return `<article class="op-card" data-id="${esc(item.id)}">
      ...
      <div class="meta"><span>◉ ${esc(item.verification)}</span><span>⌖ ${esc(item.city)}</span><span>¥ ${esc(item.startupCost)}</span><span>◷ ${esc(item.time)}</span></div>
